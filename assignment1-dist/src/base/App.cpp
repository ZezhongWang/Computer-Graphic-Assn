...
vector<Vertex> loadUserGeneratedModel() {
	...
	for(auto i = 0u; i < faces; ++i)	{
		...
		v0.position = Vec3f(FW::cos(angle_increment * i) * radius, 0, FW::sin(angle_increment * i) * radius);
		v1.position = Vec3f(FW::cos(angle_increment * (i - 1)) * radius, 0, FW::sin(angle_increment * (i - 1)) * radius);
		v2.position = Vec3f(0, height, 0);
		// Calculate the normal of the face from the positions and use it for all vertices.
		v0.normal = v1.normal = v2.normal = cross(v0.position - v2.position, v1.position - v2.position).normalized();
		//
		// Some hints:
...
:   common_ctrl_			(CommonControls::Feature_Default & ~CommonControls::Feature_RepaintOnF5),
	...
	model_translation_		(Vec3f(0, 0, 0))
{
...
void App::render() {
	...
	modelToWorld = modelToWorld * translation;
	/*if (print_flag_) {	// debug
		printf("Translation:\n");
		translation.print();
		printf("modelToWorld:\n");
		modelToWorld.print();
